...
(function (global) {
  const SKIP = /(SUB\s*-?\s*TOTAL|TOTAL|\bTAX\b|IMPUESTO|CHANGE|CAMBIO|\bCASH\b|VISA|MASTER\s*CARD|\bMC\b|AMEX|DISCOVER|DEBIT|CREDIT|TENDER|BALANCE|\bAUTH|APPROV|\bREF\b|ACCOUNT|\bACCT|CHIP|\bAID\b|\bTVR\b|\bTSI\b|ENTRY|CARD\s*#|THANK|GRACIAS|RETURN|REFUND|POLICY|RECEIPT|SURVEY|WWW\.|\.COM|STORE\s*#|\bSTR\b|\bREG\b|\bTRANS|\bTRN\b|CASHIER|ITEMS?\s+(SOLD|PURCH)|SAVINGS|YOU SAVED|\bBAG\b|PHONE|TEL\b)/i;

  const PRICE_END = /(-?\$?\s?\d{1,4}[.,]\s?\d{2})(\s*-)?\s*([A-Z*]{1,2})?\s*$/;
  ...
  function cleanName(line, codeRaw, priceRaw) {
    ...
    s = s.replace(QTY_AT, " ").replace(/\s+[A-Z*]{1,2}\s*$/, "").replace(/[$]/g, "");
  ...
  function parse(text) {
    ...
    if (/ROSS/i.test(all)) out.store = "Ross";
    ...
    const pay = all.match(/(VISA|MASTER\s*CARD|MASTERCARD|AMEX|AMERICAN EXPRESS|DISCOVER|DEBIT|CASH)/i);
